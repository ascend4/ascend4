...
# This asc variant allows a font specification.
#
...
proc asctk_dialog {w font title text bitmap default args} {
    ...
    text $w.msg 
    $w.msg insert end $text
    $w.msg configure -height [expr int([$w.msg index end]) + 1]
    $w.msg configure -width [ascdialog_width $w.msg]
    if {$tcl_platform(platform) != "unix"} {
	$w.msg configure -font system
    } else {
	$w.msg configure -font $font
    }
    catch {asc_export_selection $w.msg}
    catch {emacs-bind $w.msg}
    bind $w.msg <F3> "event generate $w.msg <<Copy>>"
    bind $w.msg <Control-C> "event generate $w.msg <<Paste>>"
...
proc ascdialog_width {w} {
  if {![winfo exists $w]} {return}
  set max 20
  set h [$w cget -height]
  for  {set i 1} {$i < $h} {incr i} {
    set wid [lindex [split [$w index $i.end] .] 1]
    if {$wid > $max} {
      set max $wid
    }
  }
