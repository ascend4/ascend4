...
class UnitsDialog:
	...
	def __init__(self,browser,displaymodel,T=None):
		...
		self.units = self.browser.ascwrap.library.getUnits()
		self.realtypes = self.browser.ascwrap.library.getRealAtomTypes()
		
		if not len(self.realtypes):
			self.browser.ET.reportError("No dimensioned atom types available yet (have you loaded a model yet?)")
	...
	def update_applybutton(self):
		...
		for k,v in self.changed.iteritems():
			T = self.browser.ascwrap.library.findType(k)
	...
	def on_typecombo_changed(self,widget,*args):
		...
		try:
			T = self.browser.ascwrap.library.findType(widget.get_active_text())
	...
	def run(self):
		...
		while _res == gtk.RESPONSE_APPLY:
			...
			if _res == gtk.RESPONSE_APPLY or _res == gtk.RESPONSE_CLOSE:
				...
				if typename:
					self.update_unitsview(self.browser.ascwrap.library.findType(typename))
				else:
					self.update_unitsview(None)
				#self.displaymodel.update_all_units()
		self.window.hide()