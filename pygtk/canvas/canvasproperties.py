...
class CanvasProperties(object):
		...
		if self.parent is not None:
			for stream in self.parent.ascwrap.streams:
				...
				for prop in stream.stream_properties:
					self.stream_store.append(row, [stream, prop, stream.stream_properties[prop]])
	...
	def draw_stream_view(self):
		...
		self.name_column = Gtk.TreeViewColumn('Name', self.name_render, text=1, foreground =4, weight=5)
		self.prop_column = Gtk.TreeViewColumn('Value', self.prop_render, text=2, foreground =4, weight=5)
