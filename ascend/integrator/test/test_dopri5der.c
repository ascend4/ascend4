...
static int test_dopri5_reporter_init(struct IntegratorSystemStruct *integ){
	return 0;
}

static int test_dopri5_reporter_write(struct IntegratorSystemStruct *integ){
	return 1; /* no interrupt */
}

static int test_dopri5_reporter_writeobs(struct IntegratorSystemStruct *integ){
	CONSOLE_DEBUG("x = %f", var_value(integ->x));
	return 0;
}

static int test_dopri5_reporter_close(struct IntegratorSystemStruct *integ){
	return 0;
}

static IntegratorReporter test_dopri5_reporter = {
	test_dopri5_reporter_init
	,test_dopri5_reporter_write
	,test_dopri5_reporter_writeobs
	,test_dopri5_reporter_close
};

/*
	Test solving a simple DOPRI5 model.
*/
static void test_shmder(){
	Asc_CompilerInit(1);
	CU_TEST(0 == Asc_PutEnv(ASC_ENV_LIBRARY "=models"));
	CU_TEST(0 == Asc_PutEnv(ASC_ENV_SOLVERS "=solvers/qrslv" OSPATH_DIV "solvers/dopri5"));
...
#define MODELNAME "shm_der"
	...
	CU_ASSERT_FATAL(0 == integrator_set_engine(integ,"DOPRI5"));
	...
	integrator_set_reporter(integ, &test_dopri5_reporter);
...
REGISTER_TESTS_SIMPLE(integrator_dopri5der, TESTS)
