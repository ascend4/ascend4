...
# define MAXLEN 3000
	...
	if(res==ERROR_SUCCESS){
		res = RegQueryValueEx(key, ASC_ENV_SOLVERS, NULL, &datatype, (unsigned char *)value, &len);
		if(res==ERROR_SUCCESS){
			RegCloseKey(key);
			return value;
		}
...
# define MAXLEN 3000
	...
	res = RegOpenKeyEx(HKEY_LOCAL_MACHINE, "SOFTWARE\\ASCEND", 0L, KEY_QUERY_VALUE, &key);
	if(res==ERROR_SUCCESS){
		res = RegQueryValueEx(key, ASC_ENV_LIBRARY, NULL, &datatype, (unsigned char *)value, &len);
		if(res==ERROR_SUCCESS){
			RegCloseKey(key);
			return value;
		}
