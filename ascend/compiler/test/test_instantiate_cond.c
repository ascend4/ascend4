...
static void instantiate_case(const char *modelname, int expect_error){
	...
	m = Asc_OpenModule("test/instantiate/cond_unselected.a4c", &status);
	(void)m;
	CU_ASSERT(status == 0);
...
static void test_cond_unselected_case(void){
	instantiate_case("cond_unselected_case", 0);
}

#define TESTS(T) \
	T(cond_unselected_case)

REGISTER_TESTS_SIMPLE(compiler_instantiate_cond, TESTS)
