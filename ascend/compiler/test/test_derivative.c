...
static void test_test1(void){
	...
	CU_ASSERT((inst = ChildByChar(root,AddSymbol("der(y,t)"))) && InstanceKind(inst)==REAL_ATOM_INST); 
        WriteDerInfo(stdout,inst);
	CU_ASSERT((inst = ChildByChar(root,AddSymbol("der(der(b.a.x,t),t)"))) && InstanceKind(inst)==ARRAY_INT_INST); 
        CU_ASSERT(inst = ChildByChar(root,AddSymbol("t")));
        WriteDerInfo(stdout,inst); 
	...
	CU_ASSERT(inst = ChildByChar(inst,AddSymbol("lower_bound"))); 
	pe = Initialize(GetSimulationRoot(sim),name,"sim1", ASCERR, WP_STOPONERR, NULL, NULL);
	CU_ASSERT(pe==Proc_all_ok);
