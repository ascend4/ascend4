...
*//** @addtogroup system_analysis
	...
	This file is almost a copy of analyze.c. The main difference
	between these two files is that k12_classify_instance and
	k12_system_generate_diffvars called from k12_analyze_make_problem
	use DerInfo instead of ode_type and ode_id. A global variable
	g_use_dersyntax defined in system.h can be used for choosing
	between k12_analyze and analyze.
...
/*
	k12_classify_instance : to be called by PushInterfacePtrs.
...
void *k12_classify_instance(struct Instance *inst, VOIDPTR vp){
  ...
  case BOOLEAN_ATOM_INST:
    ...
    if(boolean_var(inst) ) {
      ...
      ip->u.dv.obsid = IntegerChildValue(inst,OBSID_A);
...
void ProcessSolverEvents(struct e_event *event,struct Instance *i){
  ...
  for (c=1;c<=len;c++) {
    ...
    ProcessValueListEvent(ValueList,value,k12_g_symbol_values_list);
    ref = GetCaseReferences(cur_case);
    lref = gl_length(ref);
    rels = gl_create(lref);  /* maybe allocating less than needed (models) */
    logrels = gl_create(lref);  /* maybe allocating less than needed */
