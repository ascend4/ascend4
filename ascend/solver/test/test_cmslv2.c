...
static void test_cmslv_mode(const char *filenamestem, const char *optsolver,
	...
	/* assign solver */
	const char *solvername = "CMSlv2";
	int index = slv_lookup_client(solvername);
	if(index == -1){
		sim_destroy(siminst);
		Asc_CompilerDestroy();
		CU_FAIL("CMSlv2 solver package loaded but did not register solver 'CMSlv2'.");
		return;
	}
	...
	if(expect_boundary_progress
			...
			|| progress_expect == CMSLV_PROGRESS_BOUNDARY_LOCAL_CMSLV2
			|| progress_expect == CMSLV_PROGRESS_FLUIDBED_SWITCH_CMSLV2){
		CU_ASSERT_PTR_NOT_NULL(strstr(progress,"solver=CMSlv2"));
		if(expect_boundary_progress){
...
static void test_cmslv2_resolve_converged_noop(void){
	...
	cmslv_index = slv_lookup_client("CMSlv2");
...
REGISTER_TESTS_SIMPLE(solver_cmslv2, TESTS);
