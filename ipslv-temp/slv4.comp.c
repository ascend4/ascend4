...
int slv4_register(SlvFunctionsT *f)
{
...
#define SLV4(s) ((slv4_system_t)(s))
#define SERVER (sys->slv)
#define slv4_PA_SIZE 46 /* MUST INCREMENT WHEN ADDING PARAMETERS */
#define slv4_RA_SIZE 11

/* do not delete (or extend) this array definition.
*/
#define IEX(n) slv4_iaexpln[(n)]
#define slv4_IA_SIZE 16
static char *slv4_iaexpln[slv4_IA_SIZE] = {
...
/* change  slv4_PA_SIZE above (MUST INCREMENT) WHEN ADDING PARAMETERS */
...
#define TRUNCATE_COMP_PTR	(sys->parm_array[43])
#define TRUNCATE_COMP		((*(int *)TRUNCATE_COMP_PTR))
#define TRUNCATE_ALL_PTR	(sys->parm_array[44])
#define TRUNCATE_ALL		((*(int *)TRUNCATE_ALL_PTR))
#define TOWARD_COMP_BOUNDS_PTR	(sys->parm_array[45])
#define TOWARD_COMP_BOUNDS	((*(real64 *)TOWARD_COMP_BOUNDS_PTR))
/* change  slv4_PA_SIZE above (MUST INCREMENT) WHEN ADDING PARAMETERS */


#define REX(n) slv4_raexpln[(n)]
static
char *slv4_raexpln[slv4_RA_SIZE] = {
...
struct slv4_system_structure {
  ...
  void *parm_array[slv4_PA_SIZE];      /* array of pointers to param values */
  struct slv_parameter pa[slv4_PA_SIZE];/* &pa[0] => sys->p.parms */
...
static int check_system(slv4_system_t sys)
/*
 *  Checks sys for NULL and for integrity.
 */
{
  if( sys == NULL ) {
    FPRINTF(stderr,"ERROR:  (slv4) check_system\n");
 ...
 *  Outputs a vector.
 */
static void debug_out_vector( FILE *fp, slv4_system_t sys,
 ...
 */
static void debug_out_var_values( FILE *fp, slv4_system_t sys)
{
  int32 col;
 ...
 */
static void debug_out_rel_residuals( FILE *fp, slv4_system_t sys)
{
  int32 row;
 ...
 *  the jacobian matrix.
 */
static void debug_out_jacobian( FILE *fp, slv4_system_t sys)
{
  mtx_coord_t nz;
  real64 value;
 ...
 */
static boolean calc_objective( slv4_system_t sys)
{
  calc_ok = TRUE;
 ...
 */
static boolean calc_objectives( slv4_system_t sys)
{
  int32 len,i;
 ...
 */
static boolean calc_inequalities( slv4_system_t sys)
{
 ...
 */
static boolean calc_residuals( slv4_system_t sys)
{
  int32 row;
  struct rel_relation *rel;
  double time0;
 ...
 */
static boolean calc_J( slv4_system_t sys)
{
  int32 row;
 ...
 */
static void calc_nominals( slv4_system_t sys)
{
  int32 col;
  FILE *fp = MIF(sys);
  ...
  if(strcmp(SCALEOPT,"NONE") == 0 ||
	...
	if( n == 0.0 ) {
	  n = TOO_SMALL;
	  FPRINTF(fp,"ERROR:  (slv4) calc_nominals\n");
	  FPRINTF(fp,"        Variable ");
	  print_var_name(fp,sys,var);
 ...
 */
static void calc_weights( slv4_system_t sys)
{
  mtx_coord_t nz;
  real64 sum;
 ...
 *  Scales the jacobian.
 */
static void scale_J( slv4_system_t sys)
{
  int32 row;
  int32 col;
...
static void jacobian_scaled(slv4_system_t sys)
{
  int32 col;
  if (DUMPCNORM) {
    for( col=sys->J.reg.col.low; col <= sys->J.reg.col.high; col++ ) {
...
static void scale_variables( slv4_system_t sys)
{
  int32 col;
 ...
 */
static void scale_residuals( slv4_system_t sys)
{
  int32 row;
 ...
 */
static void calc_relnoms(slv4_system_t sys)
{
  int32 row, col;
  struct var_variable *var;
  struct rel_relation *rel;
 ...
 */
static void scale_J_iterative(slv4_system_t sys)
{
 ...
 */
static void scale_system( slv4_system_t sys )
{
  if(strcmp(SCALEOPT,"NONE") == 0){
 ...
 */
static int calc_pivots(slv4_system_t sys)
{
 ...
 */
static void calc_rhs(slv4_system_t sys, struct vector_data *vec,
 ...
 */
static void calc_gamma( slv4_system_t sys)
{
 ...
 */
static void calc_Jgamma( slv4_system_t sys)
{
 ...
 */
static void calc_newton( slv4_system_t sys)
{
   linsolqr_system_t lsys = sys->J.sys;
   int32 col;
 ...
 *  in the variables.
 */
static void calc_varstep1( slv4_system_t sys)
{
 ...
 *  in the variables.
 */
static void calc_varstep2( slv4_system_t sys)
{
 ...
 */
static void calc_phi( slv4_system_t sys)
{
 ...
 */
static void calc_2x2_system(slv4_system_t sys, struct calc_step_vars *vars)
{
 ...
 */
static void coefs_from_parm( slv4_system_t sys, struct calc_step_vars *vars)
{
 ...
 */
static real64 step_norm2( slv4_system_t sys, struct calc_step_vars *vars)
{
 ...
 */
static void adjust_parms( slv4_system_t sys, struct calc_step_vars *vars)
{
 ...
 */
static void compute_step( slv4_system_t sys, struct calc_step_vars *vars)
{
   int32 row,col;
 ...
 */
static void calc_step( slv4_system_t sys, int minor)
{
 ...
 *  a step.
 */
static void restore_variables( slv4_system_t sys)
{
   int32 col;
   real64 *vec;
 ...
 */
static real64 required_coef_to_stay_inbounds( slv4_system_t sys)
{
   real64 mincoef;
   int32 col;
   real64 *vec;
 ...
 *  near bounds.
 */
static void apply_step( slv4_system_t sys)
{
   FILE *lif = LIF(sys);
   int nproj = 0;
   ...
   if (TRUNCATE_COMP || TRUNCATE_ALL) {
     factor = factor_for_complementary_vars(sys);
   }
   ...
   for( col=sys->varstep.rng->low; col <= sys->varstep.rng->high; col++ ) {
      ...
      if (TRUNCATE_ALL) {
        dx = dx*TOWARD_COMP_BOUNDS*factor;
 ...
 */
static void step_accepted( slv4_system_t sys)
{
 ...
 */
static void change_maxstep( slv4_system_t sys, real64 maxstep)
{
   sys->maxstep = maxstep;
   sys->varstep.accurate = FALSE;
}
 ...
 */
static boolean block_feasible( slv4_system_t sys)
{
   int32 row;
 ...
 */
static void move_to_next_block( slv4_system_t sys)
{
  struct var_variable *var;
  struct rel_relation *rel;
  int32 row;
 ...
 */
static void reorder_new_block(slv4_system_t sys)
{
  int32 method;
  if( sys->s.block.current_block < sys->s.block.number_of ) {
 ...
 */
static void find_next_unconverged_block( slv4_system_t sys)
{
   do {
     move_to_next_block(sys);
#if DEBUG
 ...
 */
static void iteration_begins( slv4_system_t sys)
{
 ...
 *  the cpu time.
 */
static void iteration_ends( slv4_system_t sys)
{
 ...
 */
static void update_status( slv4_system_t sys)
{
...
static
int32 slv4_get_default_parameters(slv_system_t server, SlvClientToken asys,
...
/*    ,"GAUSS","GAUSS_EASY"  currently only works for ken */
  ...
  if (parameters->parms == NULL) {
     ...
     * an instance of slv4_system_structure has this pointer
     * already set in slv4_create
     */
    new_parms = (struct slv_parameter *)
      ascmalloc((slv4_PA_SIZE)*sizeof(struct slv_parameter));
    if (new_parms == NULL) {
      return -1;
    }
  ...
  slv_define_parm(parameters, bool_parm,
	       "truncomp", "truncate complementary variables",
               "truncate complementary variables",
	       U_p_bool(val, 1),U_p_bool(lo,0),U_p_bool(hi,1), 2);
...
static SlvClientToken slv4_create(slv_system_t server, int *statusindex)
{
  slv4_system_t sys;

  sys = (slv4_system_t)asccalloc(1, sizeof(struct slv4_system_structure) );
  if (sys==NULL) {
    *statusindex = 1;
    return sys;
  }
  ...
  slv4_get_default_parameters(server,(SlvClientToken)sys,&(sys->p));
  sys->integrity = OK;
  sys->presolved = 0;
...
static void destroy_matrices( slv4_system_t sys)
{
   if( sys->J.sys ) {
...
static void destroy_vectors( slv4_system_t sys)
{
...
static int slv4_eligible_solver(slv_system_t server)
{
  struct rel_relation **rp;
...
static
void slv4_get_parameters(slv_system_t server, SlvClientToken asys,
                         slv_parameters_t *parameters)
{
  slv4_system_t sys;
  (void) server;
...
static void slv4_set_parameters(slv_system_t server, SlvClientToken asys,
                         slv_parameters_t *parameters)
{
  slv4_system_t sys;
  (void) server;
...
static void slv4_get_status(slv_system_t server, SlvClientToken asys,
                            slv_status_t *status)
{
  slv4_system_t sys;
  (void) server;
...
static linsolqr_system_t slv4_get_linsolqr_sys(slv_system_t server,
                                               SlvClientToken asys)
{
  slv4_system_t sys;
  (void) server;
...
static linsol_system_t slv4_get_linsol_sys(slv_system_t server,
                                           SlvClientToken asys)
{
  (void) server;
  (void) asys;
 ...
 */
static void structural_analysis(slv_system_t server, slv4_system_t sys)
{
  var_filter_t vfilter;
  rel_filter_t rfilter;
  ...
  slv_check_bounds(SERVER,sys->vused,-1,"fixed ");
...
static void set_factor_options (slv4_system_t sys)
{
...
/*
  ...
  parameters.
*/
static void create_matrices(slv_system_t server, slv4_system_t sys)
{
...
static
void slv4_dump_internals(slv_system_t server, SlvClientToken sys,int level)
{
  (void) server;
  check_system(sys);
  if (level > 0) {
    FPRINTF(stderr,"ERROR:  (slv4) slv4_dump_internals\n");
    FPRINTF(stderr,"        slv4 does not dump its internals.\n");
  }
}
 ...
 */
static
int32 slv4_dof_changed(slv4_system_t sys)
{
  int32 ind, result = 0;
...
static void slv4_update_linsolqr(slv4_system_t sys)
{
  if (strcmp(FACTOR_OPTION,"SPK1/RANKI") == 0) {
    sys->J.fm = ranki_kw;
  } else if (strcmp(FACTOR_OPTION,"SPK1/RANKI+ROW") == 0) {
...
static
void slv4_presolve(slv_system_t server, SlvClientToken asys)
{
  ...
  check_system(sys);
  if( sys->vlist == NULL ) {
    FPRINTF(stderr,"ERROR:  (slv4) slv4_presolve\n");
  ...
  if(sys->presolved > 0) { /* system has been presolved before */
    if(!slv4_dof_changed(sys) /* no changes in fixed or included flags */
...
static void slv4_resolve(slv_system_t server, SlvClientToken asys)
{
  struct var_variable **vp;
  struct rel_relation **rp;
  slv4_system_t sys;
...
static void slv4_iterate(slv_system_t server, SlvClientToken asys)
{
  slv4_system_t sys;
  FILE              *mif;
  FILE              *lif;
  ...
  if (check_system(SLV4(sys))) return;
  if( !sys->s.ready_to_solve ) {
    FPRINTF(stderr,"ERROR:  (slv4) slv4_iterate\n");
...
static void slv4_solve(slv_system_t server, SlvClientToken asys)
{
  slv4_system_t sys;
  sys = SLV4(asys);
  if (server == NULL || sys==NULL) return;
  if (check_system(sys)) return;
  while( sys->s.ready_to_solve ) slv4_iterate(server,sys);
}

static mtx_matrix_t slv4_get_jacobian(slv_system_t server, SlvClientToken sys)
{
  if (server == NULL || sys==NULL) return NULL;
  if (check_system(SLV4(sys))) return NULL;
  return SLV4(sys)->J.mtx;
}

static int slv4_destroy(slv_system_t server, SlvClientToken asys)
{
  slv4_system_t sys;
  (void) server;
  sys = SLV4(asys);
...
int slv4_register(SlvFunctionsT *sft)
{
  if (sft==NULL)  {
    FPRINTF(stderr,"slv4_register called with NULL pointer\n");
    return 1;
  }

  sft->name = "QRSlv";
  sft->ccreate = slv4_create;
  sft->cdestroy = slv4_destroy;
  sft->celigible = slv4_eligible_solver;
  sft->getdefparam = slv4_get_default_parameters;
  sft->getparam = slv4_get_parameters;
  sft->setparam = slv4_set_parameters;
  sft->getstatus = slv4_get_status;
  sft->solve = slv4_solve;
  sft->presolve = slv4_presolve;
  sft->iterate = slv4_iterate;
  sft->resolve = slv4_resolve;
  sft->getlinsol = slv4_get_linsol_sys;
  sft->getlinsys = slv4_get_linsolqr_sys;
  sft->getsysmtx = slv4_get_jacobian;
  sft->dumpinternals = slv4_dump_internals;
  return 0;
}
