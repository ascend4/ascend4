...
#define slv5_PA_SIZE 49 /* MUST INCREMENT WHEN ADDING PARAMETERS */
...
#define SIGMA_CONSTANT_PTR	(sys->parm_array[43])
#define SIGMA_CONSTANT		((*(int32 *)SIGMA_CONSTANT_PTR))
#define SIGMA_PTR		(sys->parm_array[44])
#define SIGMA		        ((*(real64 *)SIGMA_PTR))
#define MU_MAXIMUM_PTR	        (sys->parm_array[45])
#define MU_MAXIMUM		((*(int32 *)MU_MAXIMUM_PTR))
#define MUK_PTR		        (sys->parm_array[46])
#define MUK		        ((*(real64 *)MUK_PTR))
#define TRUNCATE_COMP_PTR	(sys->parm_array[47])
#define TRUNCATE_COMP		((*(int *)TRUNCATE_COMP_PTR))
#define DESCENT_PSI_PTR	        (sys->parm_array[48])
#define DESCENT_PSI  	        ((*(int *)DESCENT_PSI_PTR))
/* change  slv5_PA_SIZE above (MUST INCREMENT) WHEN ADDING PARAMETERS */
...
struct slv5_system_structure {
  ...
  real64                 nu;           /* error */
  real64                 psi;          /* potential objective */
  ...
  real64                 eta;          /* penalty of potential function  */
...
static boolean calc_newton_residuals( slv5_system_t sys)
{
  ...
  for( ; row <= sys->newton_residuals.rng->high; row++ ) {
    ...
    if (!rel) {
      ...
      r=mtx_row_to_org(sys->J.mtx,row);
      FPRINTF(stderr,"NULL relation found !!\n");
      FPRINTF(stderr,"at row %d rel %d in calc_newton_residuals\n",(int)row,r);
      FFLUSH(stderr);
    }
#endif
...
static boolean calc_perturbed_residuals( slv5_system_t sys)
{
  ...
  for( ; row <= sys->perturbed_residuals.rng->high; row++ ) {
    ...
    if (!rel) {
      ...
      r=mtx_row_to_org(sys->J.mtx,row);
      FPRINTF(stderr,"NULL relation found !!\n");
      FPRINTF(stderr,"at row %d rel %d in calc_perturbed _residuals\n",
             (int)row,r);
      FFLUSH(stderr);
    }
#endif
...
static void scale_perturbed_residuals( slv5_system_t sys)
{
  ...
  debug_out_vector(LIF(sys),sys,&(sys->perturbed_residuals));
#endif
}
...
static void calc_gamma( slv5_system_t sys)
{
   if( sys->gamma.accurate )
      return;
   if ( sys->mu > 0.0 ) {
     matrix_product(sys->J.mtx, &(sys->perturbed_residuals),
                   &(sys->gamma), -1.0, TRUE);
   } else {
...
static void calc_perturbed_newton( slv5_system_t sys)
{
   ...
   debug_out_vector(LIF(sys),sys,&(sys->perturbed_newton));
#endif
}
...
static void calc_varstep2( slv5_system_t sys)
{
   if( sys->varstep2.accurate )
      return;
   if (sys->mu > 0.0) {
      copy_vector(&(sys->perturbed_newton),&(sys->varstep2));
      sys->varstep2.norm2 = sys->perturbed_newton.norm2;
   } else {
...
static void calc_varnewstep( slv5_system_t sys)
{
   ...
   debug_out_vector(LIF(sys),sys,&(sys->varnewstep));
#endif
}
...
static void calc_phi( slv5_system_t sys)
{
  ...
  if (sys->mu > 0.0) {
    sys->phi = 0.5 *sys->perturbed_residuals.norm2;
  }
...
#if DEBUG_COMPLEMENTARY_VAR
  FPRINTF(ASCERR,"Minimum factor = %f \n",minfactor); 
#endif /* DEBUG_COMPLEMENTARY_VAR */
  return minfactor;
}
 ...
 */
static real64 required_coef_to_stay_inbounds( slv5_system_t sys, int32 v)
{
   real64 mincoef;
   int32 col;
   real64 *vec;
...
/*
 *  Adds sys->varnewstep to the variable values in block: projecting
 *  near bounds.
 */
static void apply_newton_step( slv5_system_t sys)
{
   FILE *lif = LIF(sys);
   int nproj = 0;
   ...
   for(col=sys->varnewstep.rng->low; col<=sys->varnewstep.rng->high;col++) {
      ...
      real64 dx,val,bnd;
      var = sys->vlist[mtx_col_to_org(sys->J.mtx,col)];
      dx = vec[col]*sys->varnewstep.vec[col];
      val = var_value(var);
      if (bounds_coef < 1.0) {
...
static void apply_step( slv5_system_t sys)
{
   ...
   for( col=sys->varstep.rng->low; col <= sys->varstep.rng->high; col++ ) {
      ...
      if (bounds_coef < 1.0) {
        ...
        if (TRUNCATE_COMP && (var_complementary(var)) && (!var_fixed(var)) ) {
           dx = dx*factor;
           sys->varnewstep.vec[col] = dx/vec[col];
        } else {
         if( !sys->p.ignore_bounds ) {
...
/*    ,"GAUSS","GAUSS_EASY"  currently only works for ken */
  ...
  slv_define_parm(parameters, bool_parm,
	       "descentpsi", "check descent in potential", 
               "check descent in potential",
	       U_p_bool(val, 0),U_p_bool(lo,0),U_p_bool(hi,1), 2);
  SLV_BPARM_MACRO(DESCENT_PSI_PTR,parameters);
...
static void slv5_iterate(slv_system_t server, SlvClientToken asys)
{
  ...
  real64            bounds_coef=1.0;
  real64            previous = 0.0;
  real64            oldphi, oldpsi;
  boolean           first=TRUE, bounds_ok=FALSE,
                    new_ok=FALSE, descent_ok=FALSE, psidescent_ok=FALSE;
  ...
  while (first || !bounds_ok || !new_ok || !descent_ok){
    ...
    if (first) {
      ...
      if (!bounds_ok) {
	    ...
	    sys->maxstep*calc_sqrt_D0(sys->gamma.norm2);
            ...
            if (!psidescent_ok && DESCENT_PSI) {
              real64 maxstep_coef;
              m = m + 1;
              maxstep_coef = pow(0.50,m);
#if DEBUG_ITERATION
              FPRINTF(ASCERR,"m = %d \n",m);
              FPRINTF(ASCERR,"step coefficient = %g \n",maxstep_coef);
#endif /*  DEBUG_ITERATION  */
              if (m >= 15 ) {
              ...
              if (SHOW_LESS_IMPT) {
                FPRINTF(lif,"%-40s ---> %g\n",
                "Step reduction (psi calculations not ok)", maxstep_coef);
              }
              sys->psi = oldpsi;
              restore_variables(sys);
              change_maxstep(sys, maxstep_coef*sys->maxstep);
    ...
    bounds_ok = ((!REDUCE) || (IGNORE_BOUNDS) ||
                ((bounds_coef=required_coef_to_stay_inbounds(sys,1)) == 1.0));
    if( !bounds_ok ) {
      previous = oldphi;
      continue;
    }
