 ...
 *  Date last modified: $Date: 1998/06/11 16:58:34 $
...
struct varip {
  ...
  int complementary;	      /* set in classify_instance */
...
struct relip {
  ...
  int complementary;	/* Does it contain solely complementarity pairs ? */
...
void *classify_instance(struct Instance *inst, VOIDPTR vp)
{
  ...
  case REAL_ATOM_INST:   		/* Variable or parameter or real */
    ...
    if( solver_var(inst) ) {
      ...
      ip->u.v.complementary = BooleanChildValue(inst,COMPLEMENTARY_A);
  ...
  case REL_INST:               /* Relation (or conditional or objective) */
    ...
    ip->u.r.complementary = BooleanChildValue(inst,COMPLEMENTARY_A);
#if DEBUG_COMPLEMENTARY
    if (ip->u.r.complementary) {
      FPRINTF(ASCERR,"Complementary equation \n");
    }
#endif /* DEBUG_COMPLEMENTARY  */
    ip->u.r.model = 0;
    ip->u.r.index = 0;
    return ip;
...

  ...
  for (v = 0; v < vlen; v++) {
    ...
    if (vip->u.v.complementary) flags |= VAR_COMPLEMENTARY;
...

  ...
  for (v = 0; v < vlen; v++) {
    ...
    if (rip->u.r.complementary) flags |= (REL_COMPLEMENTARY);
...
int analyze_make_problem(slv_system_t sys, struct Instance *inst)
{
  ...
  COMPLEMENTARY_A = AddSymbolL("complementary",13);
