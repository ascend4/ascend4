 ...
 */
static real64 calc_eta( slv_system_t server )
{
  rel_filter_t rfilter;
  int32 en2;
  real64 fen2;

  rfilter.matchbits = (REL_INCLUDED | REL_ACTIVE | REL_COMPLEMENTARY);
  rfilter.matchvalue = (REL_INCLUDED | REL_ACTIVE | REL_COMPLEMENTARY);

  en2 = slv_count_solvers_rels(server,&rfilter);
  if (en2 == 0) {
    fen2 = 0.0;
  } else {
    fen2 = en2;
...
static boolean calc_mu( slv5_system_t sys)
{
  ...
  for( ; row <= sys->residuals.rng->high; row++ ) {
    ...
    if (!rel) {
      ...
      r=mtx_row_to_org(sys->J.mtx,row);
      FPRINTF(stderr,"NULL relation found !!\n");
      FPRINTF(stderr,"at row %d rel %d in calc_muk\n",(int)row,r);
      FFLUSH(stderr);
    }
#endif
  ...
  for( ; row <= sys->residuals.rng->high; row++ ) {
    ...
    if (rel_active(rel) && rel_included(rel)) {
      if (rel_complementary(rel)) {
        error = error + sys->residuals.vec[row];
      } else {
        error = error + (sys->residuals.vec[row] * sys->residuals.vec[row]);
      }
    }
  }
...
static void slv5_iterate(slv_system_t server, SlvClientToken asys)
{
  ...
  while (first || !bounds_ok || !new_ok || !descent_ok){
    ...
    descent_ok = (sys->psi < oldpsi);
    if (EXACT_LINE_SEARCH)
      descent_ok = (descent_ok && (sys->psi >= previous));
  } /* end while */
