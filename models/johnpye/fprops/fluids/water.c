...
#undef D
	...
	for(i=0; i<ntd; ++i){
		...
		ASSERT_TOL(fprops_s, T, rho, P, &error, s, s*1e-8);
		ASSERT_TOL(fprops_p, T, rho, P, &error, p, p*1e-8);
		ASSERT_TOL(fprops_cv, T, rho, P, &error, cv, cv*1e-8);
		ASSERT_TOL(fprops_w, T, rho, P, &error, w, w*2e-5);
	}
	...
	for(i=0; i<ntds; ++i){
		...
		int res = fprops_sat_T(T, &p_eval, &rho_f_eval, &rho_g_eval, P);
		ASSERT(res == 0);
		...
		double s_f_eval = fprops_s(T,rho_f,P,&error);
		ASSERT_TOL_VAL(s_f_eval, s_f, 4e-6);
		double s_g_eval = fprops_s(T,rho_g,P,&error);
		ASSERT_TOL_VAL(s_g_eval, s_g, 2e-5);

		double h_f_eval = fprops_h(T,rho_f,P,&error);
		ASSERT_TOL_VAL(h_f_eval, h_f, 1e-3);
		double h_g_eval = fprops_h(T,rho_g,P,&error);
