...
// test
	PureFluid *P = incompressible_prepare(&eos_ZS55, NULL);
...
#define D P->data->corr.incomp	
	...
	do {
		...
		fprintf(out,"%e\t%e\t%e\t%e\t%e\t%e\t%e\n",T-273.0,fprops_cp(S,&fprops_err),fprops_cv(S,&fprops_err),fprops_rho(S,&fprops_err),fprops_cp(S,&fprops_err),fprops_cp(S,&fprops_err),fprops_cp(S,&fprops_err));
		T+=1;
