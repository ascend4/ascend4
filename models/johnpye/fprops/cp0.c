...
Phi0RunData *cp0_prepare_rpp(const IdealData *I, double R, double Tstar){
	Phi0RunData *N = FPROPS_NEW(Phi0RunData);
	int i, add_const_term=1;
	N->c = 0;
	N->m = 0;
	...
	case IDEAL_CP0:
		N->np = I->data.cp0.np;
		N->ne = I->data.cp0.ne;
		MSG("Preparing PHI0 data for ideal fluid rpp(np = %d, ne = %d)",N->np, N->ne);
		for(i=0; i < N->np; ++i)if(I->data.cp0.pt[i].t == 0)add_const_term = 0;
		MSG("add_const_term = %d",add_const_term);
		N->pt = FPROPS_NEW_ARRAY(Phi0RunPowTerm,N->np + add_const_term);
		for(i=0; i < N->np; ++i){
			N->pt[i].p = I->data.cp0.pt[i].t;
			MSG("p value %d = %e",i, N->pt[i].p);
            N->pt[i].a = I->data.cp0.pt[i].c;
            MSG("a value %d = %e",i, N->pt[i].a);
		}
		break;
	case IDEAL_PHI0:
		// shouldn't need?
...
double ideal_cp0_rpp(double T, const FluidData *data){
    #define sh data->cp0
    double heatcap = 0;
    int i;
    for(i = 0; i < sh->np; ++i){
    heatcap = heatcap + sh->pt[i].a*pow(T,sh->pt[i].p);
    }
    heatcap = heatcap*1000/data->M;
    #undef sh
    return heatcap;

}

double ideal_cp0_rpp_integrate(double T, const FluidData *data){
    #define sh data->cp0
    double heatcap = 0;
    int i;
    for(i = 0; i < sh->np; ++i){
    double a = sh->pt[i].p+1;
    MSG("a = %e",a);
    heatcap = heatcap + ((sh->pt[i].a*pow(T,a)) / a);
    MSG("heatcap = %e",heatcap);
    }
    heatcap = heatcap*1000/data->M;
    #undef sh
    return heatcap;
