...
#undef CP0_TEMP
	...
	for(i=0; i<n;++i){
		...
		rho = td[i].rho;
		p = td[i].p*1e6;
	 	ASSERT_TOL(fprops_p, T, rho, P, &fprops_err, p, p*1e-3);
	}
	...
	for(i=0; i<n;++i){
		...
		rho = td[i].rho;
		h = td[i].h*1e3;
	 	ASSERT_TOL(fprops_h, td[i].T+T_adj, td[i].rho, P, &fprops_err, h, 1e-4*h);
	}
	...
	for(i=0; i<n;++i){
		...
		rho = td[i].rho;
		u = td[i].u*1e3;
		ASSERT_TOL(fprops_u, T, rho, P, &fprops_err, u, 1e3*u);
	}
	...
	for(i=0; i<n;++i){
		...
		rho = td[i].rho;
		s = td[i].s*1e3;
	 	ASSERT_TOL(fprops_s, T, rho, P, &fprops_err, s, 1e-4*s);
	}
	...
	for(i=0; i<n;++i){
		...
		//fprintf(stderr,"%.20e\t%.20e\t%.20e\n",T,rho,(a - fprops_a(T,rho,fluid)));
	 	ASSERT_TOL(fprops_a, T, rho, P, &fprops_err, a, a*1e-3	);
	}

	fprintf(stderr,"\nTRIPLE POINT PROPERTIES\n\n");
	double pt,rhoft,rhogt;
	fprops_sat_T(P->data->T_t, &pt, &rhoft, &rhogt, P);
...
int helm_check_u(const PureFluid *fluid, unsigned ntd, const TestData *td){
	...
	for(i=0; i<n;++i){
		...
		rho = td[i].rho;
		u = td[i].u*1e3;
		err = u - fprops_u(T,rho,fluid,&fprops_err);
		se += err;
		sse += err*err;
...
int helm_check_s(const PureFluid *fluid, unsigned ntd, const TestData *td){
	...
	for(i=0; i<n;++i){
		...
		rho = td[i].rho;
		s = td[i].s*1e3;
		err = s - fprops_s(T,rho,fluid,&fprops_err);
		se += err;
		sse += err*err;
...
int helm_calc_offsets(double Tref, double rhoref, double href, double sref, const PureFluid *fluid){
    //TODO: Use this enum:
    FpropsError fprops_err=FPROPS_NO_ERROR;
	double h = fprops_h(Tref, rhoref, fluid,&fprops_err);
	double s = fprops_s(Tref, rhoref, fluid,&fprops_err);
...
int helm_run_saturation_tests(const PureFluid *fluid, unsigned nsd, const TestDataSat *td, int temp_unit){
	...
	for(i=0; i<n;++i){
		...
		int res = fprops_sat_T(T, &p, &rhof, &rhog, fluid);
		assert(res==0);
		...
		hf = td[i].hf*1e3;
	 	ASSERT_TOL(fprops_h, T, rhof, fluid, &fprops_err, hf, hf*tol);
		hg = td[i].hg*1e3;
	 	ASSERT_TOL(fprops_h, T, rhog, fluid, &fprops_err, hg, hg*tol);
		sf = td[i].sf*1e3;
	 	ASSERT_TOL(fprops_s, T, rhof, fluid, &fprops_err, sf, sf*tol);
		sg = td[i].sg*1e3;
	 	ASSERT_TOL(fprops_s, T, rhog, fluid, &fprops_err, sg, sg*tol);
	}
