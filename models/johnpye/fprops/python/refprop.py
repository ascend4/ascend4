...
#imports
import ctypes, fnmatch, os, sys, copy, platform
...
if platform.system() == 'Linux':
    ...
    try:
        ...
        _rp = ctypes.windll.LoadLibrary(str(refpropso))
...
class SetErrorDebug():
    ...
    def __repr__(self):
        if not '_seterrordebug' in globals():
            SetErrorDebug.off()
...
class FluidModel():
    '''return array of current loaded fluid model
...
def setpath(path='/usr/local/lib/refprop/'):
    ...
    SETUP if root is not default. Note, all fluids and mxtures to be filed
...
def _fluidextention():
    ...
    _fldext = {}
    _fldext[_fpath + 'fluids/'] = [(each[:-4].upper()) for each in os.listdir(_fpath + 'fluids/') if fnmatch.fnmatch(each, '*.FLD')]
    _fldext[_fpath + 'fluids/'].extend([each for each in os.listdir(_fpath + 'fluids/') if fnmatch.fnmatch(each, '*.PPF')])
    _fldext[_fpath + 'mixtures/'] = [(each[:-4].upper()) for each in os.listdir(_fpath + 'mixtures/') if fnmatch.fnmatch (each, '*.MIX')]
    return _fldext
    ...
    if 'icomp' in prop and 'nc' in prop:
        if prop['icomp'] == 0 or prop['icomp'] > prop['nc']:
            raise RefpropicompError ('undefined "icomp: ' +
                                             str(prop['icomp']) +
                                             '" value, select mixture component number' +
                                             ' between 1 and ' + str(prop['nc']))
...
def _inputerrorcheck(deflocals):
    checkstring = ['hmxnme', 'hrf', 'htype', 'hmix', 'path', 'routine', 'hmodij',
                        'hfmix']
            ...
            if not deflocals[each]: pass
                ...
                if len(deflocals[each]) != _nc_rec.record:
                    raise RefpropicompError ('input value ' + str(each) +
                                                     ' does not match the setup fluid' +
                                                     'selection')
                if sum(deflocals[each]) != 1:
                    raise RefpropnormalizeError ('sum input value ' + str(each) +
                                                          ' is unequal to 1')
                ...
                if isinstance(items, list):
                    for others in items:
                        if not isinstance(others, str):
                            raise RefpropinputError ('expect "list of str" or "strs' +
                                                             ''''s" input for ''' + each +
                                                             ' instead of "' +
                                                             str((others).__class__) +'"')
                elif not isinstance(items, str):
                    raise RefpropinputError ('''expect "list of str" or "strs's"''' +
                                                     'input for ' + each + ' instead of "'
                                                     + str((items).__class__) +'"')


def _outputierrcheck(ierr, herr, defname):
    #_ierr correction for Linux system, some unknown reason the value is
    # increased by 2**32
    if ierr > 9999:
        ierr = ierr - 2**32
    if ierr != 0 and str(SetErrorDebug()) == 'on':
        print('*' * 80)
        print('*' * 80)
        print('error raised')
        print('setup details')
        print(FluidModel())
        print('refprop dll call details')
        print(defname)
        print('error details')
        print(ierr)
        print(herr)
    ...
    elif ierr < 0 and str(SetWarning()) == 'on': #raise warning
...
def _checksetupmodel(model):
    ...
    if len(models) > 1 and str(SetWarning()) == 'on':
        raise SetupWarning('''Warning, calling multiple model setups (setmod,
                                    setktv and gerg04) could result in unexpected
                                    results. Furthermore these multiple model calls are
                                    not supported by function "resetup' and consequentely
                                     in the extention module "multiRP"''')


def resetup(prop):
    '''Resetup models and re-initialize  arrays.

    This will compare the input dict and re-setup refprop models if deemed
    ...
    #establish setmod['hcomp'] value
    if '_setmod_rec' in _Setuprecord.object_list:
        if _setmod_rec.record['hcomp'][0].__class__ is list:
            _setmod_hcomp = [each.upper() for each in
                                  _setmod_rec.record['hcomp'][0]]
        else:
            _setmod_hcomp = [each.upper() for each in _setmod_rec.record['hcomp']]

    #establish _setup_rec.record['hfld'] list only
    if _setup_rec.record['hfld'][0].__class__ is list:
        _setup_rec_hfld = [each.upper() for each in _setup_rec.record['hfld'][0]]
    else: _setup_rec_hfld = _setup_rec.record['hfld']


    #initialize setmod if deemed req.
    if 'setmod' in prop:
        setmod(prop['setmod']['htype'], prop['setmod']['hmix'],
                    prop['setmod']['hcomp'])
    ...
    #initialize setktv
    if 'setktv' in prop:
        setktv(prop['setktv']['icomp'], prop['setktv']['jcomp'],
                     prop['setktv']['hmodij'], prop['setktv']['fij'],
                     prop['setktv']['hfmix'])
    ...
    if 'setref' in prop:
        ...
        if not 'p0' in prop['setref']:
            prop['setref']['p0'] =100
        setref(prop['setref']['hrf'][0], prop['setref']['ixflag'],
                 prop['setref']['x0'], prop['setref']['h0'],
                 prop['setref']['s0'], prop['setref']['t0'],prop['setref']['p0'])
        if len(prop['setref']['hrf']) == 2:
            setref(prop['setref']['hrf'][1], prop['setref']['ixflag'],
                     prop['setref']['x0'], prop['setref']['h0'],
                     prop['setref']['s0'], prop['setref']['t0'],
                     prop['setref']['p0'])
...
def test(criteria=0.00001):
    ...
    def printresults(nist, calculated):
        nonlocal truefalse
        global testresult
        calculated = float(calculated)
        testresult += '\nNIST = ' + str(nist)
        testresult += '\nCalculated = ' + str(calculated)
        testresult += '\nabs relative difference = ' + str(abs((nist - calculated)
                                                                                / nist))
    ...
    printresults(0.074915638, prop['D'] * wmix * 0.062427974)
    ...
    printresults(23643.99357, prop['h'])
    ...
    printresults(298.4313203, ((prop['t'] - 273.15) * 9 / 5) + 32)
    ...
    printresults(5536.791449, prop['w'] * 1000 / 25.4 / 12)
    ...
    printresults(1.600404035, prop['D'] * wmix / 1000)
    ...
    printresults(0.038640632, prop['qkg'])
    ...
    prop = qmass(prop['q'], prop['xliq'], prop['xvap'])
    testresult += 'check quality of methane / ethane'
    printresults(0.038640617, prop['qkg'])
    ...
    prop = flsh('tp', 200, 2814.5509, x)
    testresult += 'check quality of methane / ethane'
    printresults(0.050092665, prop['q'])
    ...
    prop = printresults(319.1674999, (hvap - hliq) / wmix)

    #test no. 15
    setup('def', 'nitrogen')
    testresult += 'check Surface Tension of Nitrogen'
    printresults(4.050420292, surft(100, [1])['sigma'] * 1000)
    ...
    printresults(101.7947826, prop['tcx'] * 1000)
    ...
    printresults(481.2761563, Dliq * wmix)
...
def setup(hrf, *hfld, hfmix='HMX.BNC'):
    ...
    Inputs 'in string format':
        hrf - reference state for thermodynamic calculations
            ...
            'iir' : h = 200, s = 1.0 for sat liq at o C (IIR convention) Other
        ...
        *hfld - file names specifying fluid mixture components
        ...
        hfmix--file name [character*255] containing parameters for the binary
            mixture model'''
    global _setmodvalues, _setup_rec, _setref_rec, _purefld_rec
    global _setktv_rec, _setmod_rec, _fixicomp, _nc_rec, _setupprop
    global _gerg04values, _gerg04_rec
    defname = sys._getframe(0).f_code.co_name, locals()

    #define setup record for def resetup
    _setup_rec = _Setuprecord(locals(), '_setup_rec')

    _setupprop = {}

    _inputerrorcheck(locals())
    _hrf.value = hrf.upper().encode('ascii')
    if hfmix == 'HMX.BNC': _hfmix.value = (_fpath + 'fluids/HMX.BNC'
                                                        ).encode('ascii')
    ...
    #reset _purefld from record
    if '_purefld_rec' in _Setuprecord.object_list:
        del _purefld_rec

    #reset setref from record
    if '_setref_rec' in _Setuprecord.object_list:
        del _setref_rec

    #reset setktv from record
    if '_setktv_rec' in _Setuprecord.object_list:
        del _setktv_rec
        
    #reset preos from record
    if '_preos_rec' in _Setuprecord.object_list:
        del _preos_rec
        
    #reset setaga from record
    if '_setaga_rec' in _Setuprecord.object_list:
        del _setaga_rec
    ...
    if '_setmodvalues' in globals():
        ierr, herr, defname_setmod = _setmod(_nc.value, _setmodvalues['htype'],
                                    _setmodvalues['hmix'], _setmodvalues['hcomp'])
        _setmodvalues['hcomp'] = _setmodvalues['hcomp'][:_nc.value]
        if _nc.value == 1 or _setmodvalues['htype'] == 'NBS':
            _setmodvalues.__delitem__('hmix')
        if _setmodvalues['htype'] == 'NBS':
            _setmodvalues.__delitem__('hcomp')
        _setupprop['setmod'] = _setmodvalues
    ...
    if '_gerg04values' in globals():
        ierr, herr, defname_gerg04 = _gerg04(_nc.value, _gerg04values['ixflag'])
        _setupprop['gerg04'] = _gerg04values
    ...
    if _hfld.value.decode('utf-8').__contains__('.MIX|'):
        if len(listhfld) > 1:
            raise RefpropinputError ('too many standard mixture input, can only' +
                                             'select one')
...
def setmod(htype='NBS', hmix='NBS', *hcomp):
    ...
    global _setmod_pre_rec, _setmodvalues

    _inputerrorcheck(locals())

    if htype.upper() != 'NBS':
        #define setup record for def resetup
        _setmod_pre_rec = _Setuprecord(locals(), '_setmod_pre_rec')

        #create list of *hcomp (input in either list format or *arg string format)
        listhcomp = ['NSB'] * _maxcomps
        if len(hcomp) == 0: pass
        elif hcomp[0].__class__ is list:
            for each in range(min(_maxcomps, len(hcomp[0]))):
                listhcomp[each] = hcomp[0][each].upper()
        else:
            for each in range(min(_maxcomps, len(hcomp))):
                listhcomp[each] = hcomp[each].upper()
        _setmodvalues = {'htype':htype.upper(), 'hmix':hmix.upper(),
                            'hcomp':listhcomp}
    elif '_setmod_pre_rec' in _Setuprecord.object_list:
        del _setmod_pre_rec, _setmodvalues


def _setmod(nc, htype, hmix, hcomp):
    global _setmod_rec, _setmod_pre_rec
    defname = sys._getframe(0).f_code.co_name, locals()
...
def gerg04(ixflag=0):
    ...
    global _gerg04_pre_rec, _gerg04values

    _inputerrorcheck(locals())

    if ixflag == 1:
        #define setup record for def resetup
        _gerg04_pre_rec = _Setuprecord(locals(), '_gerg04_pre_rec')

        _gerg04values = {'ixflag':ixflag}
    elif ixflag == 0:
        if '_gerg04_pre_rec' in _Setuprecord.object_list:
            del _gerg04_pre_rec
    else: #ixflag value not 0 or 1
        raise RefpropinputError('''ixflag value for function "gerg04" should
                                            either be 0 (default) or 1''')


def _gerg04(nc, ixflag):
    global _gerg04_rec, _gerg04_pre_rec
    defname = sys._getframe(0).f_code.co_name, locals()
...
def setref(hrf='DEF', ixflag=1, x0=[1], h0=0, s0=0, t0=273, p0=100):
    ...
    #define setup record for def resetup
    _setref_rec = _Setuprecord(locals(), '_setref_rec')
...
def critp(x):
    ...
    _inputerrorcheck(locals())
    for each in range(_nc_rec.record): _x[each] = x[each]
    if platform.system() == 'Linux':
        _rp.critp_(_x,
                        ctypes.byref(_tcrit),
                        ctypes.byref(_pcrit),
...
def therm3(t, D, x):
    ...
    if platform.system() == 'Linux':
        ...
        _rp.THERM3dll(ctypes.byref(_t),
                        ...
                        ctypes.byref(_pint),
                        ctypes.byref(_spht))
    return _prop(x = x, D = D, t = t, xkappa = _xkappa.value, beta = _beta.value,
            xisenk = _xisenk.value, xkt = _xkt.value, betas = _betas.value,
            bs = _bs.value, xkkt = _xkkt.value, thrott = _thrott.value,
            pint = _pint.value, spht = _spht.value)
...
def purefld(icomp=0):
    ...
    #define setup record for def resetup
    _purefld_rec = _Setuprecord(locals(), '_purefld_rec')
...
def satd(D, x, kph=2):
    ...
    for each in range(_nc_rec.record):
        _x[each] = x[each]
    if platform.system() == 'Linux':
...
def flsh(routine, var1, var2, x, kph=1):
    ...
    outputs:
        ...
        w--speed of sound [m/s]
            Cp, w are not defined for 2-phase states in such cases, a flag =
            -9.99998d6 is returned'''
    defname = sys._getframe(0).f_code.co_name, locals()
...
def flsh2(routine, var1, var2, x, kq=1, ksat=0, tbub=0, tdew=0, pbub=0, pdew=0,
    ...
    if routine.upper() == 'TP':
        return _prop(x = x, t = var1, p = var2, Dliq = _Dliq.value,
                Dvap = _Dvap.value, xliq = xliq, xvap = xvap, q = _q.value,
                ierr = _ierr.value, herr = _herr.value, defname = defname)
    elif routine.upper() == 'DH':
        return _prop(x = x, D = var1, h = var2, Dliq = _Dliq.value,
                Dvap = _Dvap.value, xliq = xliq, xvap = xvap, q = _q.value,
                t = _t.value, p = _p.value, ierr = _ierr.value, herr = _herr.value,
                defname = defname)
    elif routine.upper() == 'DS':
        ...
        if ksat == 0:
            return _prop(x = x, t = var1, h = var2, Dliq = _Dliq.value,
                    ksat = ksat, Dvap = _Dvap.value, xliq = xliq, xvap = xvap,
                    q = _q.value, p = _p.value, ierr = _ierr.value,
                    herr = _herr.value, pbub = _pbub.value, pdew = _pdew.value,
                    Dlbub = _Dlbub.value, Dvdew = _Dvdew.value, xbub = xbub,
                    xdew = xdew, defname = defname)
        elif ksat == 1:
            return _prop(x = x, t = var1, h = var2, Dliq = _Dliq.value,
                    ksat = ksat, Dvap = _Dvap.value, xliq = xliq, xvap = xvap,
                    q = _q.value, p = _p.value, pbub = _pbub.value,
                    pdew = _pdew.value, Dlbub = _Dlbub.value, Dvdew = _Dvdew.value,
                    xbub = xbub, xdew = xdew, ierr = _ierr.value, herr = _herr.value,
                    defname = defname)
    elif routine.upper() == 'TS':
        if ksat == 0:
            return _prop(x = x, t = var1, s = var2, Dliq = _Dliq.value,
                    ...
                    pbub = _pbub.value, pdew = _pdew.value, Dlbub = _Dlbub.value,
                    Dvdew = _Dvdew.value, xbub = xbub, xdew = xdew, ksat = ksat,
                    defname = defname)
        elif ksat == 1:
            return _prop(x = x, t = var1, s = var2, Dliq = _Dliq.value,
                    Dvap = _Dvap.value, xliq = xliq, xvap = xvap, q = _q.value,
                    ...
                    Dlbub = _Dlbub.value, Dvdew = _Dvdew.value, xbub = xbub,
                    xdew = xdew, ierr = _ierr.value, herr = _herr.value, ksat = ksat,
                    defname = defname)
    elif routine.upper() == 'TE':
        if ksat == 0:
        ...
        if ksat == 0:
            return _prop(x = x, p = var1, D = var2, Dliq = _Dliq.value,
                    ...
                    tbub = _tbub.value, tdew = _tdew.value, Dlbub = _Dlbub.value,
                    Dvdew = _Dvdew.value, xbub = xbub, xdew = xdew, ksat = ksat,
                    defname = defname)
            return prop
        elif ksat == 1:
...
def xmole(xkg):
    ...
    outputs:
        ...
        wmix--molar mass of the mixture [g/mol], a.k.a. "molecular weight"'''
    _inputerrorcheck(locals())
    for each in range(_nc_rec.record): _xkg[each] = xkg[each]
    if platform.system() == 'Linux':
        _rp.xmole_(_xkg,
                    _x,
                    ctypes.byref(_wmix))
...
def limitx(x, htype='EOS', t=0, D=0, p=0):
    ...
    if platform.system() == 'Linux':
        ...
        _rp.LIMITXdll(ctypes.byref(_htype),
                            ...
                            ctypes.c_long(3),
                            ctypes.c_long(255))
    return _prop(x = [_x[each] for each in range(_nc_rec.record)], t = _t.value,
            D = _D.value, htype = htype.upper(), p = _p.value, tmin = _tmin.value,
            tmax = _tmax.value, Dmax = _Dmax.value, pmax = _pmax.value,
            ierr = _ierr.value, herr = _herr.value, defname = defname)


def limitk(htype='EOS', icomp=1, t=0, D=0, p=0):
    ...
    if platform.system() == 'Linux':
        ...
        _rp.LIMITKdll(ctypes.byref(_htype),
                            ...
                            ctypes.c_long(3),
                            ctypes.c_long(255))
    return _prop(icomp = icomp, t = _t.value, D = _D.value, htype = htype.upper(),
            p = _p.value, tmin = _tmin.value, tmax = _tmax.value,
...
def limits(x, htype='EOS'):
    ...
    if platform.system() == 'Linux':
        ...
        _rp.LIMITSdll(ctypes.byref(_htype),
                            ...
                            ctypes.byref(_pmax),
                            ctypes.c_long(3))
    return _prop(x = [_x[each] for each in range(_nc_rec.record)],
...
def setktv(icomp, jcomp, hmodij, fij=([0] * _nmxpar),
              hfmix='HMX.BNC'):
    ...
    #define setup record for def resetup
    _setktv_rec = _Setuprecord(locals(), '_setktv_rec')
...
def setaga():
    ...
    #define setup record for def resetup
    _setaga_rec = _Setuprecord(locals(), '_setaga_rec')
