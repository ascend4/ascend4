...
int fprops_corr_avail(const EosData *E, const char *corrtype){
	...
	}else if(strcmp(corrtype,"incompressible")==0){
		switch(E->type){
		case FPROPS_IDEAL:
		case FPROPS_HELMHOLTZ:
		case FPROPS_CUBIC:
		case FPROPS_INCOMPRESSIBLE:
			return FPROPS_INCOMPRESSIBLE;
		default:
			return 0;
		}
	}
...
PureFluid *fprops_prepare(const EosData *E,const char *corrtype){
	...
	case FPROPS_IDEAL:
		...
		break;
	case FPROPS_INCOMPRESSIBLE:
		P = incompressible_prepare(E,NULL);
		break;
	default:
...
/*
// added for evaluating conductivity and viscosity of incompressible fluids
#define EVALFN_INCOMP_TRANSPORT(VAR) \
	double fprops_##VAR(FluidState state, FpropsError *err){\
...
EVALFN_INCOMP_TRANSPORT(lam); EVALFN_INCOMP_TRANSPORT(mu);	// added for incompressible 
*/
EVALFN(rho); // added for incompressible
