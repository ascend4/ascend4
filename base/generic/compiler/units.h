 ...
 *  other units functions may be called.  Must be called after dimensions
 *  table initiatialization.
 */
 ...
 *  to one and wild dimensions.  It is given the name "?".
 *
 *  c should not contain any spaces!  It may add c to the symbol table if
 *  it is not already stored there.
 *
 *  memory leak if hitting an empty hash bucket. buckets never deallocated
 ...
 *  10		bad fraction exponent.  pos is at the left of the fraction
 *              field
