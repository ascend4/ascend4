...
class PytestDisplayLauncher(unittest.TestCase):
    ...
    def test_gprofng_profiles_tests_inside_selected_display(self):
        for options in (["--gprofng"], ["--gprofng", "profile.er"], ["--gprofng=profile.er"]):
            for visible in ([], ["--visible"]):
                with self.subTest(options=options, visible=visible):
                    run, _, code, _ = self.launch(options + ["pytest"] + visible + ["test/", "-q"])
                    self.assertEqual(code, 0)
                    command = run.call_args.args[0]
                    if not visible:
                        self.assertEqual(command[:2], ["/usr/bin/xvfb-run", "-a"])
                        command = command[2:]
                    self.assertEqual(command[:4], ["/usr/bin/gprofng", "collect", "app", "-o"])
                    if options == ["--gprofng"]:
                        self.assertTrue(pathlib.Path(command[4]).name.startswith("a4-gprofng-"))
                        self.assertEqual(pathlib.Path(command[4]).suffix, ".er")
                    else:
                        self.assertEqual(str(command[4]), "profile.er")
                    self.assertEqual(command[6:], ["-m", "pytest", "test/", "-q", "--no-isolation"])

    def test_gprofng_does_not_consume_launcher_subcommands(self):
        preprocess = runpy.run_path(str(LAUNCHER))["preprocess_gprofng_args"]
        for action in ("open", "gui", "run", "int", "script", "cutest", "complete",
                       "completion", "pytest", "canvas", "units", "solvers", "env", "q"):
            with self.subTest(action=action):
                self.assertEqual(preprocess(["a4", "--gprofng", action]),
                                 ["a4", "--gprofng=", action])
