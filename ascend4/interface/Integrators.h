 ...
 *
 *         Changes:
 *              8/95      Added support code for blsode, an
...
enum Integrator_type { /* the integrators that are supported */
  BLSODE,
  /* SDASSL, */
  UNKNOWN
};
...
extern int Asc_IntegInstIntegrable( struct Instance * ,enum Integrator_type);
/*
 *  takes an instance and a type of integrator and determines
 *  if the instance can be integrated using this integrator.
 */

extern int Asc_IntegCheckStatus( slv_status_t );
/*
...
/*
 *  The Open commands may return NULL, in which case do not write to
 ...
 *  runs or changes in the same run.
 *
 *  Integ_Open_Y_File returns the pointer to a file to which state
 ...
 *  the one most recently set via Integ_SetyfileCmd.
 *
 *  Integ_Open_Obs_File returns the pointer to a file to which state
 *  variables should be written at each recorded time interval. Other
 *  info may be written here as well, but that is up to the designer
 *  of the specific integrator interface. The filename used will be
 *  the one most recently set via Integ_SetobsfileCmd.
 *
 *  If no names or empty names have been set via Integ_SetyfileCmd and
 *  Integ_SetobsfileCmd, the file returned will be NULL.
 *
 *  The FILE * most recently opened, if any, by these commands is
 *  accessible by IntegGet_*_File until Integ_Release_*_File is called.
 *  Integ_Release_*_File should be called just before closing the files
 *  though it may be called sooner if you want to keep the file pointer
 *  returned all to yourself.
 *
 *  Closing the files is your job.
 */

extern void Asc_IntegPrintYHeader(FILE *, struct Integ_system_t *);
extern void Asc_IntegPrintObsHeader(FILE *, struct Integ_system_t *);
/*
 *  If FILE is NULL, returns immediately.
 *  If FILE ok, but blsys NULL prints warning to stderr and returns.
 *  Otherwise, prints either Y or OBServations header info to the file given.
...
extern void Asc_IntegPrintYLine(FILE *, struct Integ_system_t *);
extern void Asc_IntegPrintObsLine(FILE *, struct Integ_system_t *);
/*
 *  If FILE is NULL, returns immediately.
 *  If FILE ok, but blsys NULL prints warning to stderr and returns.
 *  Otherwise, prints either Y or OBServations line to the file given.
...
/*
 *  Tcl: integrate_set_y_file filename
 *  Set the next filename to be used for integration state variable
 *  logging. does not check the filesystem sanity of the given name.
 */

extern int Asc_IntegSetObsFileCmd(ClientData ,Tcl_Interp *,
                              int argc, CONST84 char *argv[]);
/*
 *  Tcl: integrate_set_obs_file filename
 *  Set the next filename to be used for integration observation variable
 *  logging. does not check the filesystem sanity of the given name.
 */

extern int Asc_IntegSetFileUnitsCmd(ClientData ,Tcl_Interp *,
                               int argc, CONST84 char *argv[]);
/*
 *  Tcl: integrate_logunits <display,si>
...
extern int Asc_IntegSetFileFormatCmd(ClientData ,Tcl_Interp *,
                                   int argc, CONST84 char *argv[]);
/*
 *  Tcl: integrate_logformat <fixed,variable>
...
extern int Asc_IntegSetXSamples(long, double *, dim_type *);
/*
...
extern double Asc_IntegGetStepZero(struct Integ_system_t *);
/*
 *  Returns the length of the initial step user specified,
 *  or 0.0 if none was set.
 */

extern double Asc_IntegGetStepMin(struct Integ_system_t *);
/*
 *  Returns the length of the longest allowable step.
 *  or 0.0 if none was set by user.
 */

extern double Asc_IntegGetStepMax(struct Integ_system_t *);
/*
 *  Returns the length of the shortest allowable step.
 *  or 0.0 if none was set by user.
 */

extern int Asc_IntegGetMaxSteps(struct Integ_system_t *);
/*
 *  Returns the most internal steps allowed between
 *  two time samples,  or 0 if none was set by user.
 */

extern long Asc_IntegGetNSamples();
/*
 *  Returns the number of values currently stored in xsamples.
 */

extern double Asc_IntegGetXSamplei(long);
extern void Asc_IntegSetXSamplei(long,double);
/*
 *  for (i=0; i< Integ_get_nsamples(); i++):
 *  Asc_IntegGetXSamplei(i) returns the value stored in xsamples[i].
 *  Asc_IntegSetXSamplei(i,val) sets the value stored in xsamples[i] to val.
 *  Both will whine if xsample[i] does not exist.
 *
 *  No, there is no way to get back the pointer to the xsamples vector.
 */

extern dim_type *Asc_IntegGetXSamplesDim();
/*
...
extern int Asc_IntegGetXSamplesCmd(ClientData cdata, Tcl_Interp *interp,
                               int argc, CONST84 char *argv[]);
/*
 *  TCL: integrate_get_samples [display]
 ...
 *  Only the first letter of the optional argument display is significant.
 */

extern int Asc_IntegSetXSamplesCmd(ClientData ,Tcl_Interp *,
                               int argc, CONST84 char *argv[]);
/*
 *  TCL integrate_set_samples <units> <value [value...] value>
 ...
 *  of the list, otherwise at least 2 values are required.
 *
 ...
 *  Also, the units given must already exist: we do not create them.
 */


extern int Asc_IntegInstIntegrableCmd(ClientData ,Tcl_Interp *,
                                  int argc, CONST84 char *argv[]);
/*
 *  usage: integrate_able <instance> <NAME>
 *  returns "1" if instance is integrable by the named integrator, "0"
 *  otherwise. Currently valid names are: lsode
 *  The instance examined is "current", "search", or "solver".
 */
...
extern double *Asc_IntegGetDY(double *);
/*
 *  vector=IntegGet_d_y(vector)
...
extern void Asc_IntegSetDY(double *);
/*
 *  IntegSet_d_y(vector)
 *  Sets d.y[] to values in vector.
 */

extern double *Asc_IntegGetDDydx(double *);
/*
...
extern void Asc_IntegSetDDydx(double *);
/*
 *  IntegSet_d_dydx(vector)
 *  Sets d.dydx[] to values in vector.
 */

extern double *Asc_IntegGetDObs(double *);
/*
...
extern int Asc_IntegCleanupCmd(ClientData ,Tcl_Interp *,
                             int argc, CONST84 char *argv[]);
/*
 *  integrate_cleanup
 *  do any C level housekeeping need after calling integration.
 *  Always use integrate_setup/integrate_cleanup in pairs.
 */
