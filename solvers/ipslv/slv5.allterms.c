...
#define DEBUG FALSE
#define DEBUG_COMPLEMENTARY_VAR FALSE
#define DEBUG_OBJ_VALUES FALSE
#define DEBUG_OBJECTIVES TRUE
#define DEBUG_ITERATION TRUE
#define DEBUG_CENTERING FALSE
#define DEBUG_CENTERING_VALUES TRUE

#define SLV5(s) ((slv5_system_t)(s))
#define SERVER (sys->slv)
#define slv5_PA_SIZE 39 /* MUST INCREMENT WHEN ADDING PARAMETERS */
...
#define LINTIME			((*(int32 *)LINTIME_PTR))
#define ITER_BIS_LIMIT_PTR      (sys->parm_array[27])
#define ITER_BIS_LIMIT          ((*(int32 *)ITER_BIS_LIMIT_PTR))
#define BIS_TOL_PTR             (sys->parm_array[28])
#define BIS_TOL                 ((*(real64 *)BIS_TOL_PTR))
#define METHODOPT_PTR		(sys->parm_array[29])
#define METHODOPT		((*(char **)METHODOPT_PTR))
#define MEHRO_STEP_LENGTH_PTR	(sys->parm_array[30])
#define MEHRO_STEP_LENGTH	((*(char **)MEHRO_STEP_LENGTH_PTR))
#define OBJECTIVE_FUNCTION_PTR	(sys->parm_array[31])
#define OBJECTIVE_FUNCTION	((*(char **)OBJECTIVE_FUNCTION_PTR))
#define SCALE_OBJECTIVE_PTR	(sys->parm_array[32])
#define SCALE_OBJECTIVE		((*(int32 *)SCALE_OBJECTIVE_PTR))
#define MAX_SEARCH_POWER_PTR    (sys->parm_array[33])
#define MAX_SEARCH_POWER        ((*(int32 *)MAX_SEARCH_POWER_PTR))
#define SIGMA_CONSTANT_PTR	(sys->parm_array[34])
#define SIGMA_CONSTANT		((*(int32 *)SIGMA_CONSTANT_PTR))
#define SIGMA_PTR		(sys->parm_array[35])
#define SIGMA		        ((*(real64 *)SIGMA_PTR))
#define MU_MAXIMUM_PTR	        (sys->parm_array[36])
#define MU_MAXIMUM		((*(int32 *)MU_MAXIMUM_PTR))
#define MUK_PTR		        (sys->parm_array[37])
#define MUK		        ((*(real64 *)MUK_PTR))
#define TRUNCATE_VARS_PTR       (sys->parm_array[38])
#define TRUNCATE_VARS           ((*(int *)TRUNCATE_VARS_PTR))
/* change  slv5_PA_SIZE above (MUST INCREMENT) WHEN ADDING PARAMETERS */
...
struct slv5_system_structure {
  ...
  real64                 norm;         /* unscaled norm of residuals */
  real64                 comp;         /* no. of complementary terms  */
  real64                 comp_eqns;    /* no. of complementary eqns  */
};
 ...
 *  --------------------
 *     calc_number_of_complementary_terms(sys)
 *     calc_comp(sys)
 *     calc_eta(sys)
 *     ok = calc_residuals(sys)
 *     ok = calc_complementary_terms(sys)
 *     ok = calc_mu(sys)
...
/*
 * Calculates the residuals of unincluded complementary terms.
 * Returns true iff calculations preceded without error
 */
static real64 calc_number_of_complementary_terms( slv5_system_t sys)
{
  struct rel_relation **rp;
  static rel_filter_t rfilter;
  real64 terms;

  rfilter.matchbits = (REL_INCLUDED | REL_ACTIVE | REL_COMP_TERM);
  rfilter.matchvalue = (REL_ACTIVE | REL_COMP_TERM);
  terms = 0.0;
...
static void calc_comp( slv5_system_t sys )
{
  ...
  for( ; row <= sys->residuals.rng->high; row++ ) {
    ...
    if (rel_active(rel) && rel_included (rel) && rel_complementary(rel)) { 
      comp_eqns = comp_eqns + 1.0;
    }

    if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_EQN") == 0 || 
         strcmp(OBJECTIVE_FUNCTION,"NORM_OF_RESIDUALS") == 0 ) {
      if (rel_active(rel) && rel_included (rel) && rel_complementary(rel)) { 
        eqns = eqns + 1.0;
      }
    } else {
  ...
  if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_PAIR") == 0 ) {
    terms =  terms + calc_number_of_complementary_terms(sys);
  }
  ...
  if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_EQN") == 0  ||
    ...
    sys->comp = eqns;
#if DEBUG_ITERATION
  FPRINTF(ASCERR," No. of complementary equations = %g \n",eqns);
#endif /*  DEBUG_ITERATION  */
  } else {
    sys->comp = terms;
#if DEBUG_ITERATION
  FPRINTF(ASCERR," No. of complementary terms = %g \n",terms);
#endif /*  DEBUG_ITERATION  */
  }

  sys->comp_eqns = comp_eqns;
 ...
 * Returns true iff calculations preceded without error
 */
static boolean calc_complementary_terms( slv5_system_t sys)
{
  struct rel_relation **rp;
  static rel_filter_t rfilter;
...
/*
 * Calculates the summation of the log10 of the complementary terms
 */
static real64 calc_log_of_complementary_terms( slv5_system_t sys)
{
  struct rel_relation **rp;
  static rel_filter_t rfilter;
  real64 sum_terms;
  ...
  for (rp=sys->rlist;*rp != NULL; rp++) {
    if (rel_apply_filter(*rp,&rfilter)) {
      sum_terms = sum_terms + log10(rel_residual(*rp));
    }
  }
...
static boolean calc_mu( slv5_system_t sys)
{
  ...
  if (sys->comp_eqns > 0.0) {
    muk = muk / sys->comp_eqns;
  } else {
    muk = 0.0;
  }
...
#if DEBUG_CENTERING_VALUES
  FPRINTF(ASCERR,"sys->comp_eqns is = %g \n",sys->comp_eqns);
  FPRINTF(ASCERR,"muk is = %g \n",sys->mu);
#endif /* DEBUG_CENTERING_VALUES */
...
static boolean calc_perturbed_residuals( slv5_system_t sys)
{
  ...
  for( ; row <= sys->perturbed_residuals.rng->high; row++ ) {
    ...
    if ( (strcmp(METHODOPT,"MONTERO") == 0) ) {
    ...
    } else { /* MEHROTRA LINEAR IN ALPHA*/
      if ( strcmp(MEHRO_STEP_LENGTH,"LINEAR_IN_ALPHA") == 0 ) {

        sys->perturbed_residuals.vec[row]= 
                                        relman_eval(rel,&calc_ok,SAFE_CALC);
#if DEBUG
        FPRINTF(ASCERR,"calc_perturbed_residuals \n");
        FPRINTF(ASCERR,"row = %d\n",row);
        FPRINTF(ASCERR,"residual vector = %g\n",
        ...
        FPRINTF(ASCERR,"correction = %g \n",sys->correction.vec[row]);
#endif /* DEBUG */ 
        if (rel_complementary(rel) && rel_active(rel)  
            && rel_included(rel)) {  
          sys->perturbed_residuals.vec[row] = 
                                          sys->perturbed_residuals.vec[row]
	                                  - sys->sigmamu 
      ...
      } else { /* MEHROTRA QUADRATIC IN ALPHA */
  ...
  for(row = sys->residuals.rng->low; row <= sys->residuals.rng->high; row++) {
    ...
    if (rel_complementary(rel) && rel_active(rel) && rel_included(rel)) {
      ...
      FPRINTF(ASCERR,"rel_residual = %g \n",rel_residual(rel));
        ...
        sys->norm = sys->norm + (rel_residual(rel) *rel_residual(rel) );
    } else {
...
#if DEBUG_OBJECTIVES
    FPRINTF(ASCERR,"Norm of Residuals in nu = %g \n",sys->residuals.norm2);
#endif /*  DEBUG_OBJECTIVES  */
...
static void calc_psi( slv5_system_t sys)
{
  ...
  struct rel_relation *rel;
  int32 row, r;
  real64 sum, sumt, sum_terms;

  calc_nu( sys);

  if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_PAIR") == 0 ) {
    calc_complementary_terms(sys);
    sum_terms = calc_log_of_complementary_terms(sys);
#if DEBUG_OBJ_VALUES
    FPRINTF(ASCERR,"Calculating log of terms in calc_psi\n");
#endif /* DEBUG_OBJ_VALUES */ 
  }
  ...
  for(row = sys->residuals.rng->low; row <= sys->residuals.rng->high; row++) {
    ...
    if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_EQN") == 0 || 
      ...
      if (rel_active(rel) && rel_included (rel) && rel_complementary(rel)
          && rel_comp_term(rel)) {
#if DEBUG_OBJ_VALUES
          FPRINTF(ASCERR,"Complementary term in calc_psi\n");
          FPRINTF(ASCERR,"residual vector = %g\n",sys->residuals.vec[row]);
          FPRINTF(ASCERR,"rel_residual = %g \n",rel_residual(rel));
#endif /* DEBUG_OBJ_VALUES */ 
        sumt = log10(rel_residual(rel));
  ...
  if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_PAIR") == 0 ) {
      sum = sum + sum_terms;
#if DEBUG_OBJECTIVES
      FPRINTF(ASCERR,"Calculating sum of log of terms in calc_psi\n");
#endif /* DEBUG_OBJECTIVES */ 
  }

  if ( (strcmp(OBJECTIVE_FUNCTION,"NORM_OF_RESIDUALS") == 0) ||
       (sys->comp == 0.0) || (sys->eta == 0.0) ||
       (!TRUNCATE_VARS) ) {
    if (SCALE_OBJECTIVE) {
      sys->psi =  0.5*sys->residuals.norm2;
    } else {
      sys->psi =  0.5 * (sqrt(sys->norm));
    }
#if DEBUG_OBJECTIVES
    FPRINTF(ASCERR,"Norm of Residuals in psi = %g \n",sys->residuals.norm2);
    FPRINTF(ASCERR,"Psi is norm of Residuals\n");
#endif /*  DEBUG_OBJECTIVES  */
  } else {
    if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_EQN") == 0 ) {
      sys->psi = ( sys->eta * log10(sys->nu) ) - sum;
#if DEBUG_OBJECTIVES
      FPRINTF(ASCERR,"Psi is potential function for each equation \n");
#endif /*  DEBUG_OBJECTIVES  */
    } else { /* POTENTIAL_LOG_FOR_EACH_PAIR */
      sys->psi = ( sys->eta * log10(sys->nu) ) - sum;
#if DEBUG_OBJECTIVES
      FPRINTF(ASCERR,"Psi is potential function for each pair \n");
#endif /*  DEBUG_OBJECTIVES  */
    }
  }
...
static real64 quadratic_factor_for_complementary_vars( slv5_system_t sys) 
{
   ...
   real64 *vec;
   real64 dx, dxp, dxc, val, bnd, try;
   real64 factor, minfactor, fup, flow;
   real64 error;
   int32 col;
   int32 conv_flag, iter;
   ...
   for( col = corrector.rng->low; col <= corrector.rng->high; col++ ) {
     ...
     if (var_complementary(var) && (!var_fixed(var))) {
       ...
       fup = 1.0;
       flow = 0.0;
       dxp = predictor.vec[col] * vec[col];
       dxc = corrector.vec[col] * vec[col];
       error = BIS_TOL * vec[col];
       ...
       if( val + dx < bnd ) {
         ...
         while (conv_flag == 0) {
           ...
           iter++;
           if (iter > ITER_BIS_LIMIT) {
             FPRINTF(ASCERR,"quadratic_factor_for_complementary_vars \n");
             FPRINTF(ASCERR,
                  "Reached Maximum number of iterations for bisection \n");
             FPRINTF(ASCERR,"Returning zero \n");
             factor = 0.0;
             return factor;
	   }
	 }
 ...
 */
static void apply_quadratic_step( slv5_system_t sys, real64 factor)
{
   FILE *lif = LIF(sys);
   struct var_variable *var;
   real64 dx, dxp, dxc, val, bnd;
   struct vec_vector predictor, corrector;
   int32 col;
   real64 *vec;
   ...
   for(col = corrector.rng->low; col <= corrector.rng->high;col++) {
     var = sys->vlist[mtx_col_to_org(sys->J.mtx,col)];
     dxp = vec[col] * predictor.vec[col];
     dxc = vec[col] * corrector.vec[col];
     val = var_value(var);
     dx = factor * dxp + pow(factor,2)*dxc;
...
/*
 *  Adds  step to the variable values in block. It projects
 *  non complementary variables near bounds.
 */
...
/*    ,"GAUSS","GAUSS_EASY"  currently only works for ken */
  ...
  static char *objective_names[] = {
    "POTENTIAL_LOG_FOR_EACH_EQN","POTENTIAL_LOG_FOR_EACH_PAIR",
    "NORM_OF_RESIDUALS"
  };

  static char *meh_search_names[] = {
    "LINEAR_IN_ALPHA","QUADRATIC_IN_ALPHA"
  };
  ...
  slv_define_parm(parameters, bool_parm,
	       "truncomp", "truncate complementary vars", 
	       "truncate complementary vars",
	       U_p_bool(val, 1),U_p_bool(lo,0),U_p_bool(hi,1), 2);
  SLV_BPARM_MACRO(TRUNCATE_VARS_PTR,parameters);

  slv_define_parm(parameters, bool_parm,
	       "sigmaconstant", "sigma constant", "sigma constant",
	       U_p_bool(val, 0),U_p_bool(lo,0),U_p_bool(hi,1), 2);
  SLV_BPARM_MACRO(SIGMA_CONSTANT_PTR,parameters);

  slv_define_parm(parameters, bool_parm,
	       "mumaximum", "mu maximum", "mu maximum",
	       U_p_bool(val, 0),U_p_bool(lo,0),U_p_bool(hi,1), 2);
  SLV_BPARM_MACRO(MU_MAXIMUM_PTR,parameters);
  ...
  slv_define_parm(parameters,real_parm,
	       "bistol","tolerance for bisection" ,
               "tolerance for bisection" ,
	       U_p_real(val,1e-08),U_p_real(lo,0),U_p_real(hi,1.0), 1);
  SLV_RPARM_MACRO(BIS_TOL_PTR,parameters);
  ...
  slv_define_parm(parameters, real_parm,
	       "sigmaconst", "sigma (centering parameter)", 
               "sigma (centering parameter)",
	       U_p_real(val, 0.5),U_p_real(lo, 0.0),U_p_real(hi,1.0),1);
  SLV_RPARM_MACRO(SIGMA_PTR,parameters);

 slv_define_parm(parameters, real_parm,
	       "mumax", "mu maximum (centering parameter)", 
               "mu maximum (centering parameter)",
	       U_p_real(val, 0.9),U_p_real(lo, 0.0),U_p_real(hi,1.0),1);
  SLV_RPARM_MACRO(MUK_PTR,parameters);
  ...
  slv_define_parm(parameters, char_parm,
	       "ipmethod","interior point method","interior point method",
	       U_p_string(val,method_names[0]),
	       U_p_strings(lo,method_names),
	       U_p_int(hi,sizeof(method_names)/sizeof(char *)),1);
  SLV_CPARM_MACRO(METHODOPT_PTR,parameters);

  slv_define_parm(parameters, char_parm,
	       "alphasearch","search in corrector step",
	       "search in corrector step",
	       U_p_string(val,meh_search_names[1]),
	       U_p_strings(lo,meh_search_names),
	       U_p_int(hi,sizeof(meh_search_names)/sizeof(char *)),1);
  SLV_CPARM_MACRO(MEHRO_STEP_LENGTH_PTR,parameters);

  slv_define_parm(parameters, char_parm,
	       "objfunction","objective function in step length search",
               "objective function in step length search",
	       U_p_string(val,objective_names[0]),
	       U_p_strings(lo,objective_names),
	       U_p_int(hi,sizeof(objective_names)/sizeof(char *)),1);
  SLV_CPARM_MACRO(OBJECTIVE_FUNCTION_PTR,parameters);

  slv_define_parm(parameters, bool_parm,
	          "scaleobjective", "scale potential objective function", 
		  "scale potential objective function",
	          U_p_bool(val, 0),U_p_bool(lo,0),U_p_bool(hi,1), 2);
  SLV_BPARM_MACRO(SCALE_OBJECTIVE_PTR,parameters);

  slv_define_parm(parameters, int_parm,
	          "maxsearchpower", "maximum power for search", 
	          "maximum power for search",
	          U_p_int(val, 10),U_p_int(lo, 1),U_p_int(hi,20000),1);
  SLV_IPARM_MACRO(MAX_SEARCH_POWER_PTR,parameters);
...
static void slv5_iterate(slv_system_t server, SlvClientToken asys)
{
  ...
  if (SCALE_OBJECTIVE) {
    ...
    calc_psi(sys);
#if DEBUG
    FPRINTF(ASCERR,"psi is with scaled residuals\n");
#endif /*  DEBUG  */
  } else {
    calc_psi(sys);
#if DEBUG
    FPRINTF(ASCERR,"psi is with UNscaled residuals\n");
#endif /*  DEBUG  */
    scale_system(sys);
  }
  ...
  if (factor >= 0.9995) {
    sys->sigma_const = sys->sigma_const/10.0;
  }
  ...
  if (SIGMA_CONSTANT) {
    sys->sigma = sys->sigma_const;
    FPRINTF(ASCERR,"using sigma constant = %g \n",sys->sigma);
  }

  if (MU_MAXIMUM) {
    if (sys->mu > MUK) {
      sys->mu = MUK;
      FPRINTF(ASCERR,"using mu maximum = %g \n",sys->mu);
    }
  }

  if (sys->sigma > 1.0) {
    FPRINTF(ASCERR,"ERROR:  (slv5) slv5_iterate\n");
    FPRINTF(ASCERR,"sigma is greater than 1.0 \n");
    sys->s.diverged = 1;
    iteration_ends(sys);
    update_status(sys);
    return;
  ...
  restore_variables(sys);
  if ( (strcmp(METHODOPT,"MEHROTRA") == 0) ) {
  ...
  if ( (strcmp(METHODOPT,"MEHROTRA") == 0) ) {
    if ( strcmp(MEHRO_STEP_LENGTH,"QUADRATIC_IN_ALPHA") == 0 ) {
      factor = quadratic_factor_for_complementary_vars(sys);
      if (factor == 0.0) {
        FPRINTF(ASCERR,"ERROR:  (slv5) slv5_iterate\n");
        FPRINTF(ASCERR,"Error in bisection method\n");
        sys->s.diverged = 1;
        iteration_ends(sys);
        update_status(sys);
        return;
  ...
  while (first || !new_ok || !descent_ok){
    if (first) {
      ...
      if (!new_ok || !descent_ok ) {
        ...
        FPRINTF(ASCERR,"factor = %g \n",factor);
#endif /*  DEBUG_ITERATION  */
        if (m >= MAX_SEARCH_POWER) {
          FPRINTF(ASCERR,"ERROR:  (slv5) slv5_iterate\n");
          FPRINTF(ASCERR,"Coefficient m is too big = %d \n",m);
          sys->s.diverged = 1;
          iteration_ends(sys);
          update_status(sys);
          return;
    ...
    if ((sys->comp) > sys->eta) {
      FPRINTF(ASCERR,"ERROR:  (slv5) slv5_iterate\n");
      FPRINTF(ASCERR,"n2 is greater than eta \n");
      sys->s.diverged = 1;
      iteration_ends(sys);
      update_status(sys);
      return;
    ...
    if ( (strcmp(OBJECTIVE_FUNCTION,"NORM_OF_RESIDUALS") == 0) ||
      ...
      if ( strcmp(OBJECTIVE_FUNCTION,"POTENTIAL_LOG_FOR_EACH_EQN") == 0 ) {
        sys->progress = ALPHA * factor * (1.0 - sys->sigma) *
                        (sys->eta - sys->comp);
      } else { /* POTENTIAL_LOG_FOR_EACH_PAIR */
        sys->progress = ALPHA * factor * (1.0 - sys->sigma) *
                        (sys->eta - sys->comp);
    ...
    if ( (strcmp(METHODOPT,"MEHROTRA") == 0) ) {
      if ( strcmp(MEHRO_STEP_LENGTH,"QUADRATIC_IN_ALPHA") == 0 ) {
        apply_quadratic_step(sys,factor);
      } else { /* MEHROTRA LINEAR IN ALPHA  */
