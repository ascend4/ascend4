# tkfbox.tcl --
# asctkfbox.tcl -- by ben allan (baa), derived from tkfbox.tcl.
# $Revision: 1.13 $
# $Author: ballan $
# $Date: 2000/01/25 02:25:44 $
#
# (search on 'baa' and 'asc_tkfbox' for changes)
# This file is a somewhat hacked version of tkfbox.tcl
# to allow multiple paths via the global variable asc_tkfbox(otherdirs).
# Using this variable is necessary to avoid conflicts with the platform
# dependent implementations of tk_getOpen/SaveFile. Basically, we cannot
# define a -otherdirs option in proper style because doing so will break
# the mac/windoze wrappers which do not support it.
# This file dialog will not get lost under the other windows on UNIX.
#
# $asc_tkfbox(otherdirs)
# if set, should be a tcl list of the other directory
# names to offer as alternatives in the Directory: bar of the file box.
# This does not properly handle unix paths (: separated).
#
# $asc_tkfbox(cancelled)
# Normally 0. if 1, then user pushed cancel. if empty return string and
# not 1, the user made a mistake.
#
...
#
# RCS: @(#) $Id: asctkfbox.tcl,v 1.13 2000/01/25 02:25:44 ballan Exp $
#
...
proc tkFDialog {args} {
    ...
    # baa - box title bug fix?
    update idletasks; # baa
...
proc tkFDialog_Update {w} {
    ...
    upvar #0 $dataName data
    global tk_library tkPriv asc_tkfbox; #baa
...
rSASvJTGhnhcV3EJlo3kh53ltF5nAhQAOw==}]
 # baa: adding bitmaps for ascend known file types
        set tkPriv(a4cImage) [image create bitmap -data {
#define a4c_width 12
...
static unsigned char a4c_bits[] = {
   ...
   0x77, 0x0e, 0x22, 0x04, 0x54, 0x02, 0x8e, 0x03, 0x8e, 0x03, 0x8e, 0x03};
        ...
        set tkPriv(a4lImage) [image create bitmap -data {
#define a4l_width 12
...
static unsigned char a4l_bits[] = {
   ...
   0xa1, 0x0b, 0x21, 0x08, 0xa1, 0x0b, 0x21, 0x08, 0x21, 0x08, 0xff, 0x0f};
        ...
        set tkPriv(a4sImage) [image create bitmap -data {
#define a4s_width 12
...
static unsigned char a4s_bits[] = {
   ...
   0xb5, 0x0d, 0x01, 0x00, 0xdd, 0x03, 0x01, 0x00, 0xed, 0x0d, 0x01, 0x00};
        ...
        set tkPriv(dotcImage) [image create bitmap -data {
#define c_width 12
#define c_height 12
static unsigned char c_bits[] = {
   ...
   0x05, 0x08, 0x05, 0x08, 0x0d, 0x0b, 0xf9, 0x09, 0x01, 0x08, 0xff, 0x0f};
        ...
        set tkPriv(dotccImage) [image create bitmap -data {
#define cpp_width 12
...
static unsigned char cpp_bits[] = {
   ...
   0xe5, 0x0e, 0x05, 0x00, 0x0d, 0x03, 0xf9, 0x01, 0x01, 0x00, 0xff, 0x0f};
        ...
        set tkPriv(dothImage) [image create bitmap -data {
#define h_width 12
#define h_height 12
static unsigned char h_bits[] = {
   ...
   0xf9, 0x09, 0x99, 0x09, 0x09, 0x09, 0x9d, 0x0b, 0x01, 0x08, 0xff, 0x0f};
        ...
        set tkPriv(a4oImage) [image create bitmap -data {
#define opt_width 12
...
static unsigned char opt_bits[] = {
   ...
   0xca, 0x01, 0x5a, 0x01, 0x72, 0x01, 0x22, 0x01, 0xfe, 0x01, 0x00, 0x00};
    ...
    # baa: ascend file types.
    set a4c $tkPriv(a4cImage); # baa
    set a4l $tkPriv(a4lImage); # baa
    set a4s $tkPriv(a4sImage); # baa
    set a4o $tkPriv(a4oImage); # baa
    set dotc $tkPriv(dotcImage); # baa
    set doth $tkPriv(dothImage); # baa
    set dotcc $tkPriv(dotccImage); # baa
	    ...
	    if {![info exists hasDoneFile($f)]} {
               # baa: check for known extensions. this should be more general.
                if {"[file extension $f]" != ""} {
                    ...
                    .H -
                    .cc {
                            tkIconList_Add $data(icons) $dotcc $f
                        }
                    .c {
                            tkIconList_Add $data(icons) $dotc $f
                       }
                    .h {
                            tkIconList_Add $data(icons) $doth $f
                       }
                    .a4u -
                    .a4o {
                            tkIconList_Add $data(icons) $a4o $f
                         }
                    .asc -
                    .a4c {
                            tkIconList_Add $data(icons) $a4c $f
                         }
                    .lib -
                    .a4l {
                            tkIconList_Add $data(icons) $a4l $f
                         }
                    .s -
                    .a4s {
                            tkIconList_Add $data(icons) $a4s $f
                         }
                    default
                        {
    ...
    # baa:  Add caller specified list, then add any additional
    # baa:  information from asc_tkfbox(otherdirs)
    #
    set list ""
    set dir ""
...
#   }
    ...
    # we need to save the first selectPath so it doesn't
    # disappear when the path is changed to an otherdir.
    # When we close the box, we need to unset asc_tkfbox(givendir)
    if {[info exists asc_tkfbox(givendir)] == 0} {
        set asc_tkfbox(givendir) "$data(selectPath)"
    }
    # add the otherdirs options after selectPath and given dir
    # if they happen to be the same, that will filter out later on here
    set otherdirs $data(selectPath)
    lappend otherdirs $asc_tkfbox(givendir)
    if {[info exists asc_tkfbox(otherdirs)]} {
        foreach od $asc_tkfbox(otherdirs) {
            if {$od != ""} {
                lappend otherdirs $od
            }
        }
    }
    foreach od $otherdirs {
        if {$od !=""} {
            foreach subdir [file split $od] {
                set dir [file join $dir $subdir]
                if {[lsearch $list $dir] ==-1} {
                    # append only unique directories
                    lappend list $dir
                }
            }
        }
