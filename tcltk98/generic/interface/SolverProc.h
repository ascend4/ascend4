...
extern int Asc_SolvCheckSys (ClientData cdata, Tcl_Interp *interp,
                         int argc, CONST84 char *argv[]);
/*
 *  Checks whether g_solvsys_cur is nonnull
 */

extern int Asc_SolvGetObjList(ClientData cdata, Tcl_Interp *interp,
                          int argc, CONST84 char *argv[]);
/*
 *  slv_get_obj_list <dest> 0= stdout 1 = stderr 2= list
...
extern int Asc_SolvSetObjByNum(ClientData cdata, Tcl_Interp *interp,
                           int argc, CONST84 char *argv[]);
/*
 *  slv_set_obj_by_num sets the solvers objective to list[num] where
 *  list is the solvers list of objective relations.
 *  If num is -1 the ojective is set to NULL.
 */
...
extern int Asc_SolvGetSlvParms(ClientData cdata, Tcl_Interp *interp,
                           int argc, CONST84 char *argv[]);
/*
 *  Tcl call is: slv_get_parms <solver number>
 ...
 *  returned instead.
 *  Returns: number of solver parameters were retrieved from.
 *  time_limit
 *  iteration_limit
...
extern int Asc_SolvSetSlvParms(ClientData cdata, Tcl_Interp *interp,
                           int argc, CONST84 char *argv[]);
/*
 *  TCL call: set_slv_parms <items as given below>
 *  Loads up slv parameters struct from argv.
 *  This code should be updated if slv_parameters_structure
 *  slv_tolerance_data, or slv_output_data in slv.h are changed.
 *  If a specified solver is not linked, parameters will not be set.
 *  Input  : solver number
 *  time_limit (seconds)
 ...
 *  yet, their set call remains to be defined.
 *
 *  Notes: Tcl_ExprBoolean is used on the binaries.
 */
 ...
 *  slv__block_status_structure in slv.h change.
 *  Returns:
 *  ok   (boole)
 *  over_defined   (boole)
 *  under_defined   (boole)
 ...
 *  solver. Some solvers may not have one. Each list element is:
 *    block size
 *    iterations in block
 *    function calls
 *    jacobian calls
...
extern int Asc_SolvCheckAndReanalyze(ClientData cdata, Tcl_Interp *interp,
                                 int argc, CONST84 char *argv[]);
/*
 *  check if the instance modified in the browser is a member of a
 ...
 *  If t not given, 5 cpu seconds assumed.
 */

extern int Asc_SolvAvailSolver(ClientData cdata, Tcl_Interp *interp,
                           int argc, CONST84 char *argv[]);
/*
 *  slv_available <no args> returns the list of linked solver names.
 *  if a solver in not linked, the first three char of its name wil be
 *  no_
 */
...
extern int Asc_SolvLinsolNames(ClientData cdata, Tcl_Interp *interp,
                           int argc, CONST84 char *argv[]);
/*
 *  slv_linsol_names <no args> returns the list of linsolqr factor
 *  methods separated by commas. split at commas to extract names.
 */
...
extern int Asc_SolvImportQlfdid(ClientData cdata, Tcl_Interp *interp,
                            int argc, CONST84 char *argv[]);
/*
 *  Solv_ImportQlfdid import qlfdid to solver context
...
extern int Asc_SolvMakeIndependent(ClientData cdata, Tcl_Interp *interp,
                               int argc, CONST84 char *argv[]);
/*
 *  slv_set_independent <index> [index ...]
 ...
 *  The cost data will also be updated if necessary.
 */
 ...
 *  Usage:
 *  slv_set_haltflag integer.
 *  where integer is an integer value, 0 or nonzero. If 0 will turn the
 ...
 *  slvhelp [s,l]
 *  no arg or "s" -> list names only. "l" -> short explanations also.
 */

extern int Asc_SolvNearBounds(ClientData cdata, Tcl_Interp *interp,
                          int argc, CONST84 char *argv[]);
/*
 *  slv_near_bounds for tcl
 *  see function of same name in c for documentation
 */

extern int Asc_SolvFarFromNominal(ClientData cdata, Tcl_Interp *interp,
                              int argc, CONST84 char *argv[]);
/*
 *  slv_far_from_nominals for tcl
 *  see function of same name in c for documentation
 */
